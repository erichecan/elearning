...
const FlashcardsManager: React.FC<{ childId?: string }> = ({ childId }) => {
  const [items, setItems] = useState<FlashcardItem[]>([])
  const [loading, setLoading] = useState(false)
  const [statusFilter, setStatusFilter] = useState('all')
  const [topic, setTopic] = useState('Sea animals')
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`/api/flashcards${query ? `?${query}` : ''}`)
  ...
  const bulkUpdate = async (status: string) => {
    try {
      await apiFetch('/api/flashcards/bulk-status', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 h-full overflow-y-auto">
      <div className="bg-white p-6 rounded-lg shadow mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium">Flashcards 审核</h3>
            ...
            onClick={async () => {
              ...
              try {
                await apiFetch('/api/flashcards/generate', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ topic, count, childId })
                })
                loadItems()
              } catch (error) {
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        {loading ? (
          ...
          <div className="space-y-3">
            {items.map((item) => (
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-7 gap-3 items-center border rounded-lg p-3">
                <input
                  type="checkbox"
                ...
                />
                <div>
                  <div className="font-semibold">{item.word_en}</div>
                  <div className="text-sm text-gray-500">{item.word_zh || ''}</div>
                </div>
                ...
                <div className="w-full h-16 bg-slate-100 rounded-lg overflow-hidden">
                  {item.image_url ? (
                    <img src={item.image_url} alt={item.word_en} className="h-full w-full object-cover" />
                  ) : (

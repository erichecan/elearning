...
async function generateComprehensiveMigration() {
    console.log('🏗️ 生成TotCards全面数据迁移文件...\n');

    const db = new LocalDatabase('./totcards-comprehensive.db');
    ...
    try {
        ...
        const filename = `20250629_${timestamp}_import_totcards_comprehensive.sql`;
        const supabaseMigrationPath = path.join('..', 'supabase', 'migrations', filename);
        
        let sql = '';
        sql += '-- TotCards全面数据导入\n';
        sql += `-- 导入时间: ${new Date().toISOString()}\n`;
        sql += '-- 来源: totcards.com (全分类)\n';
        sql += '-- 包含: 84个单词 (月份、数字、字母、形状、颜色、时间)\n\n';
        ...
        console.log('\n' + '='.repeat(70));
        console.log('🎉 TotCards全面迁移文件生成完成！');
...
// 运行生成器
generateComprehensiveMigration().catch(console.error); 